@extends('themes.base')
@section('subhead')
    <title>Bản Đồ Ngập Lụt - PCTT Cà Mau Dashboard</title>
@endsection
...
@push('scripts')
    <script>
        const calamitiesByFloodRange = @json($calamitiesByFloodRange);
        ...
        let geoJsonLayer = false; // Biến lưu trữ layer GeoJSON
        ...
        // Ngập lụt
        let kmlLayersFlood = new Map(); // Lưu trữ KML layers theo ID ngập lụt
        let currentFloodId = null; // Lưu ID của lớp đang hiển thị

        document.getElementById("floodingSelect").addEventListener("change", function() {
            updateFloodList(this.value);
        });

        function updateFloodList(floodRange) {
            const calamities = floodRange === "all" ? calamitiesByFloodRange["all"] : calamitiesByFloodRange[floodRange] ||
                [];
            clearAllKmlLayers(); // Xóa tất cả lớp cũ

            const floodList = document.getElementById("floodList");
            floodList.innerHTML = ""; // Xóa danh sách cũ

            if (calamities.length === 0) {
                floodList.innerHTML = "<p class='text-gray-500'>Không có dữ liệu.</p>";
                return;
            }
            ...
            calamities.forEach(calamity => {
                ...
                listItem.innerHTML = `
                    <label class="flex items-center space-x-2 bg-gray-50 p-2 rounded-md cursor-pointer">
                        <input type="radio" name="flood-layer" class="flood-checkbox" data-id="${calamity.id}" data-urls='${JSON.stringify(mapFiles)}'>
            ...
            // Bắt sự kiện click radio button - kiểm tra lại sau khi cập nhật danh sách
            document.querySelectorAll(".flood-checkbox").forEach(radio => {
                radio.addEventListener("change", function() {
                    let calamityId = this.dataset.id;
                    let fileUrls;

                    try {
                        fileUrls = JSON.parse(this.dataset.urls);
                    } catch (error) {
                        console.error("Lỗi khi parse JSON dataset.urls:", error);
                        return;
                    }

                    toggleKmlLayerFlood(calamityId, fileUrls);
                });
            });
        }

        function toggleKmlLayerFlood(calamityId, urls) {
            // Xóa lớp cũ nếu có
            if (currentFloodId !== null && kmlLayersFlood.has(currentFloodId)) {
                kmlLayersFlood.get(currentFloodId).forEach(layer => layer.setMap(null));
                kmlLayersFlood.delete(currentFloodId);
            }
            ...
            if (calamityId) {
                ...
                urls.forEach(async (url) => {
                    ...
                    try {
                        ...
                        if (!response.ok) {
                            console.error(`❌ Lỗi khi tải file KML: ${kmlUrl} (HTTP ${response.status})`);
                            return; // Bỏ qua nếu file không tồn tại
                        }
                        // console.log(`✅ File KML hợp lệ: ${kmlUrl}`);
                        ...
                        google.maps.event.addListener(kmlLayer, 'status_changed', function() {
                            ...
                            // console.log(`ℹ️ Trạng thái KML (${kmlUrl}):`, status);
                            if (status !== "OK") {
                                console.error(
                                    `⚠️ Lỗi khi hiển thị file KML: ${kmlUrl} - Trạng thái: ${status}`
                                );
                            }
                        });
                        layers.push(kmlLayer);
                    } catch (error) {
                        console.error(`🚨 Lỗi khi kiểm tra/tạo KML Layer: ${kmlUrl}`, error);
                    }
                });
                kmlLayersFlood.set(calamityId, layers);
                currentFloodId = calamityId; // Cập nhật ID lớp hiện tại
            }
        }

        function clearAllKmlLayers() {
            kmlLayersFlood.forEach(layers => {
                layers.forEach(layer => layer.setMap(null));
            });
            kmlLayersFlood.clear();
            currentFloodId = null; // Đặt lại trạng thái
        }
