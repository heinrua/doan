...
@section('subhead')
    <title>Cập Nhật Cấp Độ Thiên Tai - PCTT Cà Mau Dashboard</title>
@endsection
@php
...
@section('subcontent')
    <h2 class="intro-y mt-5 text-lg font-medium uppercase flex items-center">
        {!! $icons['chevron-right'] !!}
        Cập Nhật Cấp Độ Thiên Tai
    </h2>
    <div class="mt-5 grid grid-cols-1 gap-x-6 pb-20"> {{-- Chỉnh thành grid-cols-1 để tối ưu mobile --}}
        <div class="intro-y">
            <form enctype="multipart/form-data" class="validate-form" action="{{ route('update-risk-level') }}" method="post">
                            ...
                            <div class="flex-col md:flex-row items-start pt-5 first:mt-0 first:pt-0"
                                ...
                                <div class="w-full">
                                    <input name="name" id="name" type="text"
                                        placeholder="Tên Cấp Độ Rủi Ro Thiên Tai" value="{{ $risk_level->name }}" />
                ...
                <div class="mt-5 flex flex-col justify-end gap-2 md:flex-row">
                    <a href="{{ route('view-risk-level') }}">
                        <button type="button"
                            class="text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2">
                            Huỷ Bỏ</button>
                    </a>
                    @if ($userCurrent->is_master || $userCurrent->hasPermission('update-role'))
